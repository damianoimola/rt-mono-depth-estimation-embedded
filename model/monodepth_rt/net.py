...
class UNet(nn.Module):
    def __init__(self, in_channels, out_channels, training, mode='sum'):
        ...
        if mode == 'concat':
            ...
            if training: self.pred4 = self.prediction_decoder(512, out_channels)
            self.dec3 = self.upconv_block(512, 128)
            if training: self.pred3 = self.prediction_decoder(256, out_channels)
            self.dec2 = self.upconv_block(256, 64)
            if training: self.pred2 = self.prediction_decoder(128, out_channels)
            self.dec1 = self.upconv_block(128, 32)
            self.pred1 = self.prediction_decoder(32, out_channels)
        else:
            self.dec4 = self.upconv_block(512, 256)
            if training: self.pred4 = self.prediction_decoder(256, out_channels)
            self.dec3 = self.upconv_block(256, 128)
            if training: self.pred3 = self.prediction_decoder(128, out_channels)
            self.dec2 = self.upconv_block(128, 64)
            if training: self.pred2 = self.prediction_decoder(64, out_channels)
    ...
    def forward(self, x):
        ...
        if self.mode == 'concat':
            ...
            if self.training: p4 = self.pred4(d4)

            d3 = self.dec3(d4)
            d3 = torch.cat((d3, e2), dim=1)
            if self.training: p3 = self.pred4(d3)

            d2 = self.dec2(d3)
            d2 = torch.cat((d2, e1), dim=1)
            if self.training: p2 = self.pred4(d2)
        ...
        return [comb, p4, p3, p2, p1] if self.training else p1